...
//
import type { LineInvitationProps, AddUserProps } from "@/interface/data";
import { AddUserSchema } from "@/interface/zod";

//statements
import { checkLineInvitation } from "@/db/statements/line";
//utils
...
const LineUserInfoRegister = () => {
  const [onOpen, setOnOpen] = useState(0);

  const [uploadingFiles, setUploadingFiles] = useState<{
    [key: string]: boolean;
  }>({});
  const { lineInvitationId, sgId, unitPosId } = useParams();
  ...
  const { data, isFetching } = useQuery<LineInvitationProps>({
    queryKey: ["invitationLink", lineInvitationId],
    queryFn: () => checkLineInvitation(lineInvitationId as string),
  ...
  const handleOnSubmit = async (formInput: AddUserProps) => {
    ...
    formData.append("lineInvitationId", lineInvitationId);
    ...
    try {
      const response = await axios.post("/line/user/register", formData, {
        headers: {
  ...
  useEffect(() => {
    if (data && data.status === 1 && data.submittedApplicationId) {
      nav(
        `/line/register/${lineInvitationId}/${data.lineId}/${unitPosId}/${sgId}`,
        {
          replace: true,
          ...
          <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-8">
            <div className="text-center">
              ...
              <h1 className="text-xl font-bold text-gray-900 mb-2 mt-10">
                {data.unitPosition?.position.name || "Unknown"}
              </h1>
...
export default LineUserInfoRegister;
